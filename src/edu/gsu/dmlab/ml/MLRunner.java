...
public class MLRunner {
	...
	public MLRunner(IHistogramProducer histoProducer, String type,
			String sourceFolder, int[][] dims, int compMethod) {
		...
		this.init(sourceFolder, dims, compMethod);
	}

	private void init(String sourceFolder, int[][] dims, int compMethod) {
		File[] files = this.getFiles(new File(sourceFolder), type);
			...
			labelsFutures.add(exService.submit(new Callable<Mat[][]>() {
				...
				public Mat[][] call() throws Exception {
					...
					for (int j = idx; j < idx2; j++) {
						ITrack[] tracks = ObjectFactory.getTrackedResults("AR",
								files[j].getParent(), span);
						retMat[count++] = getLabels(tracks, dims, compMethod);
						System.out.println(files[j].getParent() + ":Done");
					}
					return retMat;
				}
			}));
		...
		this.labelsArr = new Mat[files.length][];
	...
	private Mat[] getLabels(ITrack[] tracks, int[][] dims, int compMethod) {
		...
		Mat dataMat = new Mat(dataList.size(), 2, CvType.CV_32F);
