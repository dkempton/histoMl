...
public class TopKRedundencyLimit {

	ITrack[][] trackArray;
	DbAccess dbAccess;
	ResultWriter rw;

	public TopKRedundencyLimit(ITrack[][] trackArray, DbAccess dbAccess,
			ResultWriter rw) {
	...
	public void run() {
		...
		for (int dimNum = 3; dimNum < 21; dimNum++) {
			...
			while (!foundNext && leavIdx < 90) {
				...
				double tmpRedundMean = RedundancyEval.eval(tmpDataAndLabels);
				if ((tmpRedundMean - redundMean) < (redundMean * 0.1)) {
					redundMean = tmpRedundMean;
